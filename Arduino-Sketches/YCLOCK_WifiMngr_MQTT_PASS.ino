...
//
#define MQTTSRVR "Your Broker"
#define MQTTPORT   "1883"
#define MQTTTOPIC1 "Your Topic 1"
#define MQTTTOPIC2 "Your Topic 2"
#define MQTTUSER "Your Username"
#define MQTTPASS "Your Password"
...
void p_scrollChar(int8_t xs, uint8_t ys,char ch) {
   ...
   for (uint8_t x=0;x < 6; x++) {
       ...
       d = pgm_read_byte_near( System6x7+
                               (ch-32)*6+
                               x);
    
       if ((d& 1 ) ==  1)   p_drawPixel(xs, 0+ys, 0xFF);else  p_drawPixel(xs, 0+ys, 0x00);
       if ((d& 2 ) ==  2)   p_drawPixel(xs, 1+ys, 0xFF);else  p_drawPixel(xs, 1+ys, 0x00);
       if ((d& 4 ) ==  4)   p_drawPixel(xs, 2+ys, 0xFF);else  p_drawPixel(xs, 2+ys, 0x00);
       if ((d& 8 ) ==  8)   p_drawPixel(xs, 3+ys, 0xFF);else  p_drawPixel(xs, 3+ys, 0x00);
       if ((d&16 ) == 16)   p_drawPixel(xs, 4+ys, 0xFF);else  p_drawPixel(xs, 4+ys, 0x00);
       if ((d&32 ) == 32)   p_drawPixel(xs, 5+ys, 0xFF);else  p_drawPixel(xs, 5+ys, 0x00);
       if ((d&64 ) == 64)   p_drawPixel(xs, 6+ys, 0xFF);else  p_drawPixel(xs, 6+ys, 0x00);
       p_scan(1);
       delay(SCROLL_SPEED);    
